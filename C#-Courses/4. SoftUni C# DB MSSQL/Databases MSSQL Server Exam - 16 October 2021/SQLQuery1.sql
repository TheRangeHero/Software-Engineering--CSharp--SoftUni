...
CREATE TABLE Sizes
			 ...
			 RingRange DECIMAL(4,2) CHECK(RingRange BETWEEN 1.5 AND 7.5)
			 )
...
-- PROBLEM 9
		...
		CONCAT(c.FirstName,' ',c.LastName) AS FullName
		  ...
		  JOIN Cigars AS ci
			ON cc.CigarId = ci.Id
		 WHERE a.ZIP LIKE '[0-9]%' AND a.ZIP NOT LIKE '%[^0-9]%'
		...
		CREATE FUNCTION udf_ClientWithCigars (@name VARCHAR(30))
			RETURNS INT
			      ...
			      BEGIN
					   ...
					   RETURN @totalNumberCigars
			       END

		        SELECT [dbo].udf_ClientWithCigars ('Woody')
